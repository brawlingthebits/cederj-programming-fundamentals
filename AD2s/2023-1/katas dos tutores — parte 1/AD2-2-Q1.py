...
cliente = struct.Struct("10s 40s 40s 80s")
printInfo = False
    ...
    with open(nome_arquivo_cliente, "rb") as arquivo:
        ...
        while True:
            ...
            if tipo == 'aeroporto':
                if consulta in aeroporto:
                    print(f"As {len(cias_separadas)} cias aéreas que operam para {consulta}, {cidade} - {pais}, são:")
        ...
        if tipo == 'país':
            if len(dic_aeroporto) > 0:
                print(f"Os {len(dic_aeroporto[consulta])} aeroportos de {consulta} são:")
        ...
        if tipo == 'cia':
            if len(dic_cias) > 0:
                print(f"A companhia aérea {consulta} opera para os {len(dic_cias[consulta])} aeroportos:")
